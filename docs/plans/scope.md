...
0. **Configuration and floor plan are required to qualify; the scope is
   laid down from them.** (2026-09-18, third round.) `properties.
   configuration` (2 BHK…) and a `floor_plan` document are asked in the
   stage dialog when moving to *qualified* and refused by the transition
   without them; on success the matching preset fills an empty scope.
   Moving to *Proposal* needs a rough size and at least one component on
   every space of ours. There is no Requirements section and no brief:
   services wanted are what the spaces contain, finishes live on the
   space or component, budget and timeline are the lead's fields.
