...
export async function applyChoices(
  ...
  for (const plan of args.plans) {
    ...
    // put there is already the first preference.
    const done = new Set(
    ...
    const wanted: { cost_item_id: string; quantity: number | null; group_key: string | null }[] = [
      ...
      ...plan.counted.map((c) => ({ cost_item_id: c.cost_item_id, quantity: c.quantity, group_key: null })),
    ];
  ...
  if (inserts.length) {
    ...
    // p1 moves whatever held that place to second, so a blanket applied over
    // an answer keeps the old one as the alternative rather than losing it.
