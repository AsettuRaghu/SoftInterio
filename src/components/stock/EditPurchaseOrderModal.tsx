...
export default function EditPurchaseOrderModal({
  ...
  useEffect(() => {
    if (isOpen && purchaseOrder) {
      ...
      setExpectedDelivery(purchaseOrder.expected_delivery?.split("T")[0] || "");
      setPaymentTerms(purchaseOrder.payment_terms || "");
      setNotes(purchaseOrder.notes || "");
      setShippingAddress(purchaseOrder.shipping_address || "");

      // Map existing items
      const existingItems: POItem[] = (purchaseOrder.items || []).map(
        (item) => ({
          id: item.id,
          material_id: item.material_id,
          material_name: item.material?.name || "Unknown",
          material_sku: item.material?.sku || "",
          unit: item.material?.unit_of_measure || "pcs",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/stock/purchase-orders/${purchaseOrder.id}`,
        {
          method: "PATCH",
  ...
  // Check if edit is allowed based on status
      ...
      >
        <div className="text-center py-8">
          ...
          <p className="text-sm text-slate-500 mt-2">
            Only Draft and Pending Approval orders can be edited.
          </p>
        </div>
        <ModalFooter>
          <button
          ...
          <div className="space-y-5">
            ...
            <div className="border border-slate-200 rounded-lg">
              ...
              {showMaterialDropdown &&
                ...
                createPortal(
                  <div
                    id="edit-material-dropdown-portal"
