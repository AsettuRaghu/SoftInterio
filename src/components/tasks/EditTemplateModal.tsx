...
function convertApiItemsToLocal(apiItems: any[]): TemplateItem[] {
  return apiItems.map((item) => ({
    id: item.id || generateId(),
    title: item.title,
    priority: item.priority || "medium",
    children: item.children ? convertApiItemsToLocal(item.children) : [],
    isExpanded: true,
  }));
}
...
export function EditTemplateModal({
  ...
  const fetchTemplateDetails = async () => {
    ...
    try {
      ...
      setDefaultPriority(tmpl.default_priority || "medium");
      setItems(convertApiItemsToLocal(tmpl.items || []));
    } catch (err) {
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                            <PriorityBadge
                              value={item.priority}
                              onChange={(val) =>
                                updateTaskPriority(item.id, val)
                              }
                              showLabel={false}
                              size="sm"
                            />
