-- BASELINE SCHEMA
--
-- The full public schema as it stood on 2026-09-07, captured with
-- `supabase db dump` from the linked project.
--
-- Why this exists: every migration in this project was a delta - 61 ALTERs, 55
-- UPDATEs, 36 DROPs - on a schema that had only ever been built in the
-- Supabase dashboard. Not one core table was created by a migration, so the
-- folder could not rebuild a database: a fresh replay died on the first ALTER.
-- Roughly ten months of schema evolution existed nowhere in the repository.
--
-- This file is that missing foundation. It carries 124 tables, 323 RLS
-- policies, 135 functions, 50 types and 364 indexes - including the policies
-- and the trg_lead_stage_change trigger, which had never been in version
-- control at all.
--
-- It is timestamped ahead of every delta so a replay creates the tables first,
-- and it is already marked applied in the remote ledger, so `supabase db push`
-- will not try to run it against the live database.
--
-- The 43 deltas it supersedes are kept in supabase/migrations_archive/.
--
-- Regenerate after significant dashboard-side changes:
...
ALTER TYPE "public"."procedure_action_type" OWNER TO "postgres";
...
ALTER TYPE "public"."procedure_run_status" OWNER TO "postgres";
...
ALTER TYPE "public"."project_activity_type_enum" OWNER TO "postgres";
...
ALTER TYPE "public"."project_priority_enum" OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."can_complete_task"("p_task_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    v_task  RECORD;
...
ALTER FUNCTION "public"."can_complete_task"("p_task_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."can_complete_task"("p_task_id" "uuid") IS 'Single source of truth for whether a task may complete: open subtasks plus unmet procedure requirements.';
...
CREATE OR REPLACE FUNCTION "public"."can_start_task"("p_task_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    v_prev TEXT[];
...
ALTER FUNCTION "public"."can_start_task"("p_task_id" "uuid") OWNER TO "postgres";
...
BEGIN
    ...
    -- 2. Copy documents from lead to project using unified documents table
    ...
    v_result := jsonb_build_object(
        'success', true,
        'notes_linked', v_notes_count,
        'documents_copied', v_documents_count,
        'quotations_linked', v_quotations_count
    );
...
COMMENT ON FUNCTION "public"."carry_forward_lead_data_to_project"("p_lead_id" "uuid", "p_project_id" "uuid") IS 'Carries forward lead data (notes, documents, quotations) to a new project. Documents are copied from unified documents table.';
...
CREATE OR REPLACE FUNCTION "public"."copy_quotation_to_project"("p_source_quotation_id" "uuid", "p_project_id" "uuid", "p_created_by" "uuid") RETURNS "uuid"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_new_quotation_id uuid;
  v_new_quotation_number text;
  v_source_quotation RECORD;
  v_space_mapping JSONB := '{}'::jsonb;
  v_old_space_id uuid;
  v_new_space_id uuid;
  v_component_mapping JSONB := '{}'::jsonb;
  v_old_component_id uuid;
  v_new_component_id uuid;
BEGIN
  -- Get source quotation
  SELECT * INTO v_source_quotation 
  FROM "public"."quotations" 
  WHERE id = p_source_quotation_id;
  
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Source quotation not found: %', p_source_quotation_id;
  END IF;
  
  -- Generate new quotation number for project
  v_new_quotation_number := 'PRJ_' || to_char(NOW(), 'YYYYMMDD') || '_' || LPAD(FLOOR(RANDOM() * 10000)::text, 4, '0');
  
  -- Create new quotation record (V1 of project quotation) - NO LOCKING COMPLEXITY
  INSERT INTO "public"."quotations" (
    ...
    v_source_quotation.tenant_id,
    v_new_quotation_number,
    1, -- Start at version 1 for project
    p_source_quotation_id, -- Link to original sales quotation
    v_source_quotation.lead_id,
    v_source_quotation.client_id,
    p_project_id,
    v_source_quotation.title,
    v_source_quotation.description,
    'approved'::character varying, -- Status: approved for project baseline
    v_source_quotation.valid_from,
    v_source_quotation.valid_until,
    v_source_quotation.subtotal,
    v_source_quotation.discount_type,
    v_source_quotation.discount_value,
    v_source_quotation.discount_amount,
    v_source_quotation.taxable_amount,
    v_source_quotation.tax_percent,
    v_source_quotation.tax_amount,
    v_source_quotation.overhead_percent,
    v_source_quotation.overhead_amount,
    v_source_quotation.grand_total,
    v_source_quotation.payment_terms,
    v_source_quotation.terms_and_conditions,
    v_source_quotation.notes,
    p_created_by,
    p_created_by,
    v_source_quotation.presentation_level,
    v_source_quotation.hide_dimensions,
    v_source_quotation.assigned_to,
    'sales'::public.quotation_type_enum,
    v_source_quotation.company_logo_url,
    v_source_quotation.header_color,
    v_source_quotation.show_company_details,
    v_source_quotation.show_bank_details,
    v_source_quotation.custom_footer_text,
    v_source_quotation.template_id,
    NULL -- Will be set to self-reference below
  ) RETURNING id INTO v_new_quotation_id;
  
  -- Set baseline_quotation_id to self (marks this as the baseline V1)
  UPDATE "public"."quotations"
  SET baseline_quotation_id = v_new_quotation_id
  WHERE id = v_new_quotation_id;
  
  -- Copy spaces
  FOR v_old_space_id IN
    SELECT id FROM "public"."quotation_spaces"
    WHERE quotation_id = p_source_quotation_id
  LOOP
    INSERT INTO "public"."quotation_spaces" (
      quotation_id,
      space_type_id,
      name,
      description,
      ...
      subtotal,
      display_order
    FROM "public"."quotation_spaces"
    WHERE id = v_old_space_id
    RETURNING id INTO v_new_space_id;
    
    -- Store mapping of old space id to new space id
    v_space_mapping := jsonb_set(v_space_mapping, ARRAY[v_old_space_id::text], to_jsonb(v_new_space_id::text));
  END LOOP;
  
  -- Copy components
  FOR v_old_component_id IN
    SELECT id FROM "public"."quotation_components"
    WHERE quotation_id = p_source_quotation_id
  LOOP
    INSERT INTO "public"."quotation_components" (
      quotation_id,
      space_id,
      component_type_id,
      name,
      ...
      subtotal,
      display_order
    FROM "public"."quotation_components"
    WHERE id = v_old_component_id
    RETURNING id INTO v_new_component_id;
    
    -- Store mapping of old component id to new component id
    v_component_mapping := jsonb_set(v_component_mapping, ARRAY[v_old_component_id::text], to_jsonb(v_new_component_id::text));
  END LOOP;
  
  -- Copy line items with proper space and component references
  INSERT INTO "public"."quotation_line_items" (
    ...
    v_new_quotation_id,
    CASE 
      WHEN quotation_space_id IS NOT NULL 
      THEN (v_space_mapping ->> quotation_space_id::text)::uuid
      ELSE NULL 
    END,
    CASE 
      WHEN quotation_component_id IS NOT NULL 
      THEN (v_component_mapping ->> quotation_component_id::text)::uuid
      ELSE NULL 
    END,
    quotation_cost_item_id,
    name,
  ...
  -- Return the new quotation ID
  RETURN v_new_quotation_id;
END;
$$;


ALTER FUNCTION "public"."copy_quotation_to_project"("p_source_quotation_id" "uuid", "p_project_id" "uuid", "p_created_by" "uuid") OWNER TO "postgres";
...
COMMENT ON FUNCTION "public"."create_quotation_for_lead"("p_lead_id" "uuid", "p_user_id" "uuid") IS 'Creates a new quotation or revision for a lead. If lead already has quotations, creates new version with same number.';



CREATE OR REPLACE FUNCTION "public"."create_quotation_from_lead"("p_lead_id" "uuid") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_quotation_id UUID;
...
ALTER FUNCTION "public"."create_quotation_from_lead"("p_lead_id" "uuid") OWNER TO "postgres";
...
BEGIN
    ...
    -- Create new quotation header WITHOUT redundant fields
    INSERT INTO quotations (
    ...
    -- Copy quotation_cost_attributes
...
COMMENT ON FUNCTION "public"."duplicate_quotation"("p_quotation_id" "uuid", "p_user_id" "uuid") IS 'Creates a copy/revision of a quotation with proper ID mapping. Uses client_id and lead_id relationships instead of redundant fields.';
...
BEGIN
    ...
    v_project_number := 'PRJ_' || v_date || '_' || LPAD(v_count::TEXT, 4, '0');
    
    -- If the number already exists (race condition), use timestamp-based fallback
    IF EXISTS (SELECT 1 FROM projects WHERE tenant_id = p_tenant_id AND project_number = v_project_number) THEN
        v_project_number := 'PRJ_' || to_char(NOW(), 'YYYYMMDDHH24MISS') || '_' || floor(random() * 100000)::text;
    END IF;
...
COMMENT ON FUNCTION "public"."generate_project_number"("p_tenant_id" "uuid") IS 'Generates unique project numbers in format PRJ_YYYYMMDD_XXXX with race condition detection and timestamp-based fallback';
...
DECLARE
    ...
    v_number VARCHAR(50);
BEGIN
    -- Format: QT-YYYYMMDD-XXX (matches TypeScript implementation)
    v_date_prefix := 'QT-' || TO_CHAR(CURRENT_DATE, 'YYYYMMDD') || '-';
    
    -- Count only version 1 quotations for today (first versions only)
    SELECT COUNT(*) + 1 INTO v_count
    FROM quotations
    WHERE tenant_id = p_tenant_id
    AND version = 1
    AND quotation_number LIKE v_date_prefix || '%';
    
    -- Sequential number with 3 digits
    v_number := v_date_prefix || LPAD(v_count::TEXT, 3, '0');
...
COMMENT ON FUNCTION "public"."generate_quotation_number"("p_tenant_id" "uuid") IS 'Generates date-based quotation number (QT-YYYYMMDD-XXX) for first version of new quotations';
...
CREATE OR REPLACE FUNCTION "public"."is_valid_task_transition"("p_from" "public"."task_status", "p_to" "public"."task_status") RETURNS boolean
    LANGUAGE "plpgsql" IMMUTABLE
    AS $$
BEGIN
...
ALTER FUNCTION "public"."is_valid_task_transition"("p_from" "public"."task_status", "p_to" "public"."task_status") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."is_valid_task_transition"("p_from" "public"."task_status", "p_to" "public"."task_status") IS 'Allowed task status transitions. Advisory - enforced in the task_transition() RPC, not by constraint. todo->completed is permitted and records zero worked time.';



CREATE OR REPLACE FUNCTION "public"."lock_quotation_for_project"("p_quotation_id" "uuid", "p_project_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  UPDATE "public"."quotations"
  SET 
    is_locked = true,
    linked_to_project_id = p_project_id,
    updated_at = NOW()
  WHERE id = p_quotation_id;
END;
$$;
...
ALTER FUNCTION "public"."lock_quotation_for_project"("p_quotation_id" "uuid", "p_project_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."prevent_locked_quotation_edit"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- If quotation is locked, only allow updates to metadata fields (is_locked, linked_to_project_id, baseline_quotation_id, updated_at)
  -- Deny updates to actual quotation content
  IF OLD.is_locked = true THEN
    -- Check if any content column changed (not just metadata)
    IF OLD.title IS DISTINCT FROM NEW.title OR
       ...
       OLD.tax_percent IS DISTINCT FROM NEW.tax_percent THEN
      RAISE EXCEPTION 'Cannot edit locked quotation: %', OLD.id;
    END IF;
  END IF;
  
  -- Check if quotation is linked to a project (sales side) - allow only metadata updates
  IF OLD.linked_to_project_id IS NOT NULL THEN
    -- Check if any content column changed
    IF OLD.title IS DISTINCT FROM NEW.title OR
       OLD.description IS DISTINCT FROM NEW.description OR
       OLD.status IS DISTINCT FROM NEW.status OR
       OLD.subtotal IS DISTINCT FROM NEW.subtotal OR
       ...
       OLD.discount_value IS DISTINCT FROM NEW.discount_value OR
       OLD.tax_percent IS DISTINCT FROM NEW.tax_percent THEN
      RAISE EXCEPTION 'Cannot edit quotation linked to project: %. This quotation is now managed by the project.', OLD.id;
    END IF;
  END IF;
...
ALTER FUNCTION "public"."prevent_locked_quotation_edit"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."recalc_lead_next_follow_up"("p_lead_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
    UPDATE "public"."leads"
...
ALTER FUNCTION "public"."recalc_lead_next_follow_up"("p_lead_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."recalc_project_next_follow_up"("p_project_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
...
ALTER FUNCTION "public"."recalc_project_next_follow_up"("p_project_id" "uuid") OWNER TO "postgres";
...
ALTER FUNCTION "public"."recalc_task_timing"("p_task_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."recalc_tenant_storage"("p_tenant_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_docs  bigint;
...
ALTER FUNCTION "public"."recalc_tenant_storage"("p_tenant_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."recalc_tenant_storage"("p_tenant_id" "uuid") IS 'Recompute tenant_usage.storage_used_bytes from documents + task_attachments. Called by triggers on both tables.';
...
CREATE OR REPLACE FUNCTION "public"."revoke_requirement_signoff"("p_requirement_id" "uuid", "p_user_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    v_req  RECORD;
...
ALTER FUNCTION "public"."revoke_requirement_signoff"("p_requirement_id" "uuid", "p_user_id" "uuid") OWNER TO "postgres";
...
ALTER FUNCTION "public"."sign_off_requirement"("p_requirement_id" "uuid", "p_user_id" "uuid", "p_note" "text") OWNER TO "postgres";
...
ALTER FUNCTION "public"."start_procedure_run"("p_definition_id" "uuid", "p_related_type" "public"."task_related_type", "p_related_id" "uuid", "p_user_id" "uuid", "p_start_date" "date") OWNER TO "postgres";
...
ALTER FUNCTION "public"."task_blocking_predecessors"("p_task_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."task_transition"("p_task_id" "uuid", "p_user_id" "uuid", "p_to" "public"."task_status", "p_reason" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    v_task     RECORD;
...
ALTER FUNCTION "public"."task_transition"("p_task_id" "uuid", "p_user_id" "uuid", "p_to" "public"."task_status", "p_reason" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."task_transition"("p_task_id" "uuid", "p_user_id" "uuid", "p_to" "public"."task_status", "p_reason" "text") IS 'Validated task status change. Blocks completing a parent with open subtasks, and reopens a completed parent when one of its subtasks is reopened.';
...
ALTER FUNCTION "public"."trg_lead_note_follow_up"() OWNER TO "postgres";
...
ALTER FUNCTION "public"."trg_project_note_follow_up"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."trg_tasks_cascade_link_to_subtasks"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION "public"."update_project_activities_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    NEW.updated_at = CURRENT_TIMESTAMP;
...
ALTER FUNCTION "public"."update_project_activities_updated_at"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_project_notes_timestamp"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    NEW.updated_at = now();
...
ALTER FUNCTION "public"."update_project_notes_timestamp"() OWNER TO "postgres";
...
COMMENT ON TABLE "public"."clients" IS 'Simplified client registry - essential fields only';
...
COMMENT ON COLUMN "public"."quotation_cost_item_categories"."is_charge" IS 'Items in this category are charges (delivery, cleanup, protection) rather than quoted scope. A space containing only these prints as an additional charges block instead of a numbered room.';
...
CREATE TABLE IF NOT EXISTS "public"."documents" (
    ...
    "parent_linked_type" "public"."document_linked_type",
    "parent_linked_id" "uuid"
);
...
COMMENT ON TABLE "public"."leads" IS 'Simplified leads/opportunities table with core pipeline tracking';
...
CREATE TABLE IF NOT EXISTS "public"."procedure_definitions" (
    ...
    "enforce_order" boolean DEFAULT false NOT NULL
);
...
CREATE TABLE IF NOT EXISTS "public"."procedure_runs" (
    ...
    "status" "public"."procedure_run_status" DEFAULT 'active'::"public"."procedure_run_status" NOT NULL,
    "started_by" "uuid",
...
CREATE TABLE IF NOT EXISTS "public"."procedure_step_definitions" (
    ...
    "action_type" "public"."procedure_action_type" DEFAULT 'manual'::"public"."procedure_action_type" NOT NULL,
    "form_schema" "jsonb",
...
CREATE TABLE IF NOT EXISTS "public"."projects" (
    ...
    "priority" "public"."project_priority_enum" DEFAULT 'Medium'::"public"."project_priority_enum",
    "current_phase_id" "uuid",
    "actual_start_date" "date",
    "next_follow_up_at" "date",
    CONSTRAINT "projects_overall_progress_check" CHECK ((("overall_progress" >= 0) AND ("overall_progress" <= 100))),
    CONSTRAINT "projects_status_check" CHECK ((("status")::"text" = ANY ((ARRAY['new'::character varying, 'in_progress'::character varying, 'on_hold'::character varying, 'cancelled'::character varying, 'completed'::character varying])::"text"[])))
);
...
COMMENT ON COLUMN "public"."projects"."expected_start_date" IS 'Expected/planned start date for the project. This is set from lead dates or custom input during project creation.';
...
COMMENT ON COLUMN "public"."projects"."baseline_quotation_id" IS 'Reference to the baseline (V1) quotation for this project';
...
CREATE OR REPLACE VIEW "public"."project_material_costs" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."project_notes_combined" WITH ("security_invoker"='true') AS
 SELECT "n"."id",
    "n"."lead_id",
    "n"."project_id",
    "n"."content",
...
CREATE OR REPLACE VIEW "public"."project_payment_milestones_view" WITH ("security_invoker"='true') AS
 SELECT "pm"."id",
    "pm"."project_id",
    "pm"."name",
    "pm"."description",
...
CREATE OR REPLACE VIEW "public"."project_phases_summary" WITH ("security_invoker"='true') AS
 SELECT "pp"."id",
    "pp"."project_id",
    "pp"."name",
    "pp"."category_code",
...
COMMENT ON TABLE "public"."properties" IS 'Simplified property registry - essential fields only for interior design ERP';
...
CREATE TABLE IF NOT EXISTS "public"."property_scope_items" (
    ...
    "width" numeric,
    "height" numeric,
    "measurement_unit" "text" DEFAULT 'ft'::"text" NOT NULL,
    "measurement_source" "text" DEFAULT 'discussion'::"text" NOT NULL,
    "measurement_status" "text" DEFAULT 'rough'::"text" NOT NULL,
    "notes" "text",
    "created_by" "uuid",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "component_type_id" "uuid",
    "quality_tier" "text",
    CONSTRAINT "property_scope_items_component_needs_parent" CHECK ((("component_type_id" IS NULL) OR ("parent_id" IS NOT NULL))),
    CONSTRAINT "property_scope_items_measurement_source_check" CHECK (("measurement_source" = ANY (ARRAY['discussion'::"text", 'client_cad'::"text", 'site_survey'::"text"]))),
    CONSTRAINT "property_scope_items_measurement_status_check" CHECK (("measurement_status" = ANY (ARRAY['rough'::"text", 'confirmed'::"text"]))),
    CONSTRAINT "property_scope_items_measurement_unit_check" CHECK (("measurement_unit" = ANY (ARRAY['mm'::"text", 'cm'::"text", 'inch'::"text", 'ft'::"text", 'm'::"text"]))),
    CONSTRAINT "property_scope_items_name_not_blank" CHECK (("length"(TRIM(BOTH FROM "name")) > 0)),
    CONSTRAINT "property_scope_items_no_self_parent" CHECK ((("parent_id" IS NULL) OR ("parent_id" <> "id"))),
    CONSTRAINT "property_scope_items_space_xor_component" CHECK ((NOT (("space_type_id" IS NOT NULL) AND ("component_type_id" IS NOT NULL))))
);


ALTER TABLE "public"."property_scope_items" OWNER TO "postgres";


COMMENT ON COLUMN "public"."property_scope_items"."component_type_id" IS 'Set when this row is a component inside a space. Mutually exclusive with space_type_id; a component always has a parent.';



COMMENT ON COLUMN "public"."property_scope_items"."quality_tier" IS 'Specification level for this component, matching quotation_cost_items.quality_tier. Deliberately unconstrained: the catalogue defines which tiers exist, and a hard-coded list here would drift from it.';
...
COMMENT ON COLUMN "public"."quotation_line_items"."margin_amount" IS 'Profit on this line at the time it was priced: (rate - company_cost) x the same measure that produced amount. Written by the API, never by the client. Null when company_cost is unknown - which is not the same as zero margin.';
...
CREATE TABLE IF NOT EXISTS "public"."quotation_print_formats" (
    ...
    "name" "text" NOT NULL,
    "description" "text",
    "cover_enabled" boolean DEFAULT false NOT NULL,
    "cover_image_path" "text",
    "itemise_to" "text" DEFAULT 'component'::"text" NOT NULL,
    "price_at" "text" DEFAULT 'component'::"text" NOT NULL,
    "show_descriptions" boolean DEFAULT false NOT NULL,
    "show_specifications" boolean DEFAULT false NOT NULL,
    "show_dimensions" boolean DEFAULT true NOT NULL,
    "show_quantities" boolean DEFAULT true NOT NULL,
    "show_company_details" boolean DEFAULT true NOT NULL,
    ...
    "header_color" "text" DEFAULT '#1e293b'::"text" NOT NULL,
    "footer_text" "text",
    ...
    CONSTRAINT "quotation_print_formats_itemise_to_check" CHECK (("itemise_to" = ANY (ARRAY['space'::"text", 'component'::"text", 'category'::"text", 'cost_item'::"text"]))),
    CONSTRAINT "quotation_print_formats_name_not_blank" CHECK (("length"(TRIM(BOTH FROM "name")) > 0)),
    CONSTRAINT "quotation_print_formats_price_at_check" CHECK (("price_at" = ANY (ARRAY['space'::"text", 'component'::"text", 'category'::"text", 'cost_item'::"text", 'none'::"text"])))
);


ALTER TABLE "public"."quotation_print_formats" OWNER TO "postgres";


COMMENT ON COLUMN "public"."quotation_print_formats"."terms_clause_id" IS 'The terms clause this format prints. Null falls back to the tenant default, so an unconfigured format still produces a complete document.';
...
CREATE TABLE IF NOT EXISTS "public"."quotation_spaces" (
    ...
    CONSTRAINT "quotation_spaces_measurement_unit_check" CHECK ((("measurement_unit" IS NULL) OR ("measurement_unit" = ANY (ARRAY['mm'::"text", 'cm'::"text", 'inch'::"text", 'ft'::"text", 'm'::"text"]))))
);
...
COMMENT ON COLUMN "public"."quotation_spaces"."measurement_unit" IS 'Unit the length/width/height are expressed in. Copied from the property scope row this space was generated from; null when entered directly.';
...
CREATE TABLE IF NOT EXISTS "public"."quotation_templates" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "level" "text" DEFAULT 'quotation'::"text" NOT NULL,
    CONSTRAINT "quotation_templates_level_check" CHECK (("level" = ANY (ARRAY['quotation'::"text", 'space'::"text", 'component'::"text", 'cost_items'::"text"])))
);
...
COMMENT ON COLUMN "public"."quotation_templates"."level" IS 'What this template is a template of. quotation = spaces + components + items (the original behaviour); space = one room and its contents; component = one component and its cost items; cost_items = a bundle of cost items with no component of its own.';
...
CREATE TABLE IF NOT EXISTS "public"."quotation_terms_clauses" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "quotation_terms_clauses_content_not_blank" CHECK (("length"(TRIM(BOTH FROM "content")) > 0)),
    CONSTRAINT "quotation_terms_clauses_title_not_blank" CHECK (("length"(TRIM(BOTH FROM "title")) > 0))
);


ALTER TABLE "public"."quotation_terms_clauses" OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."quotations"."parent_quotation_id" IS 'For project quotations: references the original sales quotation it was copied from';
...
COMMENT ON COLUMN "public"."quotations"."baseline_quotation_id" IS 'For project quotations: references the first version (V1/baseline) of this quotation';
...
COMMENT ON COLUMN "public"."quotations"."linked_to_project_id" IS 'For sales quotations: the project ID this quotation is linked to (read-only)';



COMMENT ON COLUMN "public"."quotations"."is_locked" IS 'True if quotation cannot be edited (linked to project or is project baseline)';
...
COMMENT ON COLUMN "public"."space_types"."is_container" IS 'Whether spaces of this type may contain other spaces. False for rooms; true for things like Floor or Block, which architecture and multi-storey work need.';
...
CREATE OR REPLACE VIEW "public"."stock_material_best_prices" WITH ("security_invoker"='true') AS
...
CASE
    WHEN ("ended_at" IS NULL) THEN (0)::bigint
    ELSE GREATEST((0)::bigint, (EXTRACT(epoch FROM ("ended_at" - "started_at")))::bigint)
END) STORED,
    "source" character varying(20) DEFAULT 'status'::character varying NOT NULL,
    "note" "text",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "task_work_sessions_range_check" CHECK ((("ended_at" IS NULL) OR ("ended_at" >= "started_at"))),
    CONSTRAINT "task_work_sessions_source_check" CHECK ((("source")::"text" = ANY (ARRAY['status'::"text", 'manual'::"text", 'timer'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."tasks" (
    ...
    "total_held_seconds" bigint DEFAULT 0 NOT NULL,
...
CREATE OR REPLACE VIEW "public"."tasks_with_details" WITH ("security_invoker"='true') AS
 SELECT "t"."id",
    "t"."tenant_id",
    "t"."task_number",
    "t"."parent_task_id",
...
CREATE OR REPLACE VIEW "public"."tasks_with_timing" WITH ("security_invoker"='true') AS
 SELECT "id",
    ...
    "first_completed_at",
    "completion_count",
    ((("total_active_seconds")::numeric + COALESCE(( SELECT "sum"(GREATEST((0)::bigint, (EXTRACT(epoch FROM ("now"() - "ws"."started_at")))::bigint)) AS "sum"
           FROM "public"."task_work_sessions" "ws"
          WHERE (("ws"."task_id" = "t"."id") AND ("ws"."ended_at" IS NULL))), (0)::numeric)))::bigint AS "live_active_seconds",
    ("total_held_seconds" +
        CASE
            WHEN ("status" = ANY (ARRAY['on_hold'::"public"."task_status", 'blocked'::"public"."task_status"])) THEN GREATEST((0)::bigint, (EXTRACT(epoch FROM ("now"() - COALESCE(( SELECT "max"("h"."changed_at") AS "max"
               FROM "public"."task_status_history" "h"
              WHERE ("h"."task_id" = "t"."id")), "created_at"))))::bigint)
            ELSE (0)::bigint
        END) AS "live_held_seconds",
    (EXISTS ( SELECT 1
           FROM "public"."task_work_sessions" "ws"
          WHERE (("ws"."task_id" = "t"."id") AND ("ws"."ended_at" IS NULL)))) AS "is_clock_running",
        CASE
            WHEN ("completed_at" IS NOT NULL) THEN (EXTRACT(epoch FROM ("completed_at" - "created_at")))::bigint
            ELSE NULL::bigint
        END AS "lead_time_seconds",
        CASE
            WHEN (("completed_at" IS NOT NULL) AND ("first_started_at" IS NOT NULL)) THEN (EXTRACT(epoch FROM ("completed_at" - "first_started_at")))::bigint
            ELSE NULL::bigint
        END AS "cycle_time_seconds",
    (( SELECT "count"(*) AS "count"
           FROM "public"."task_status_history" "h"
          WHERE (("h"."task_id" = "t"."id") AND ("h"."to_status" = 'in_progress'::"public"."task_status"))))::integer AS "start_count",
    (GREATEST((0)::bigint, (( SELECT "count"(*) AS "count"
           FROM "public"."task_status_history" "h"
          WHERE (("h"."task_id" = "t"."id") AND ("h"."to_status" = 'in_progress'::"public"."task_status"))) - 1)))::integer AS "resume_count",
        CASE
            WHEN ("first_completed_at" IS NOT NULL) THEN (EXTRACT(epoch FROM ("first_completed_at" - "created_at")))::bigint
            ELSE NULL::bigint
        END AS "original_lead_time_seconds",
    ("completion_count" > 1) AS "is_rework",
    (( SELECT "count"(*) AS "count"
           FROM "public"."tasks" "st"
          WHERE (("st"."parent_task_id" = "t"."id") AND ("st"."status" <> ALL (ARRAY['completed'::"public"."task_status", 'cancelled'::"public"."task_status"])))))::integer AS "open_subtask_count"
   FROM "public"."tasks" "t";
...
CREATE TABLE IF NOT EXISTS "public"."tenant_settings" (
    ...
    "allow_direct_project_create" boolean DEFAULT false NOT NULL
);
...
COMMENT ON COLUMN "public"."tenant_settings"."allow_direct_project_create" IS 'When false (the default), projects may only be created from a won lead. Enforced in POST /api/projects; the New Project button is hidden to match.';
...
CREATE OR REPLACE VIEW "public"."vendors" WITH ("security_invoker"='true') AS
 SELECT "id",
    "tenant_id",
    "code",
    "name",
...
ALTER TABLE ONLY "public"."procedure_definitions"
    ADD CONSTRAINT "procedure_definitions_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."procedure_step_definitions"
    ADD CONSTRAINT "procedure_step_definitions_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."quotation_print_formats"
    ADD CONSTRAINT "quotation_print_formats_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."quotation_terms_clauses"
    ADD CONSTRAINT "quotation_terms_clauses_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."task_completion_requirements"
    ADD CONSTRAINT "task_completion_requirements_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_component_types_tenant_id" ON "public"."component_types" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_cost_attribute_types_tenant_id" ON "public"."cost_attribute_types" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_documents_parent_linked" ON "public"."documents" USING "btree" ("parent_linked_type", "parent_linked_id");
...
CREATE INDEX "idx_lead_notes_followup" ON "public"."lead_notes" USING "btree" ("lead_id", "follow_up_at") WHERE (("follow_up_at" IS NOT NULL) AND ("follow_up_done_at" IS NULL));
...
CREATE INDEX "idx_leads_lead_number" ON "public"."leads" USING "btree" ("tenant_id", "lead_number");
...
CREATE INDEX "idx_procedure_definitions_tenant" ON "public"."procedure_definitions" USING "btree" ("tenant_id", "is_active");



CREATE INDEX "idx_procedure_runs_entity" ON "public"."procedure_runs" USING "btree" ("related_type", "related_id", "status");



CREATE INDEX "idx_procedure_runs_tenant" ON "public"."procedure_runs" USING "btree" ("tenant_id", "status");



CREATE INDEX "idx_procedure_steps_current" ON "public"."procedure_step_definitions" USING "btree" ("definition_id", "is_current", "display_order");



CREATE INDEX "idx_procedure_steps_definition" ON "public"."procedure_step_definitions" USING "btree" ("definition_id", "display_order");



CREATE INDEX "idx_project_activities_activity_type" ON "public"."project_activities" USING "btree" ("activity_type");



CREATE INDEX "idx_project_activities_created_at" ON "public"."project_activities" USING "btree" ("created_at" DESC);



CREATE INDEX "idx_project_activities_meeting_scheduled" ON "public"."project_activities" USING "btree" ("meeting_scheduled_at") WHERE ("meeting_scheduled_at" IS NOT NULL);
...
CREATE INDEX "idx_project_notes_created_at" ON "public"."project_notes" USING "btree" ("created_at");



CREATE INDEX "idx_project_notes_created_by" ON "public"."project_notes" USING "btree" ("created_by");



CREATE INDEX "idx_project_notes_followup" ON "public"."project_notes" USING "btree" ("project_id", "follow_up_at") WHERE (("follow_up_at" IS NOT NULL) AND ("follow_up_done_at" IS NULL));
...
CREATE INDEX "idx_project_notes_tenant_id" ON "public"."project_notes" USING "btree" ("tenant_id");



CREATE INDEX "idx_project_phase_activity_log_project_id" ON "public"."project_phase_activity_log" USING "btree" ("project_id");
...
CREATE INDEX "idx_projects_current_phase_id" ON "public"."projects" USING "btree" ("current_phase_id");
...
CREATE INDEX "idx_psi_component_type" ON "public"."property_scope_items" USING "btree" ("component_type_id");



CREATE INDEX "idx_psi_parent" ON "public"."property_scope_items" USING "btree" ("parent_id");



CREATE INDEX "idx_psi_property" ON "public"."property_scope_items" USING "btree" ("property_id", "display_order");



CREATE INDEX "idx_psi_tenant" ON "public"."property_scope_items" USING "btree" ("tenant_id");



CREATE UNIQUE INDEX "idx_qpf_one_default_per_tenant" ON "public"."quotation_print_formats" USING "btree" ("tenant_id") WHERE "is_default";



CREATE INDEX "idx_qpf_tenant" ON "public"."quotation_print_formats" USING "btree" ("tenant_id");



CREATE UNIQUE INDEX "idx_qpf_unique_name_per_tenant" ON "public"."quotation_print_formats" USING "btree" ("tenant_id", "lower"("name"));



CREATE INDEX "idx_qtc_tenant" ON "public"."quotation_terms_clauses" USING "btree" ("tenant_id");



CREATE UNIQUE INDEX "idx_qtc_unique_title_per_tenant" ON "public"."quotation_terms_clauses" USING "btree" ("tenant_id", "lower"("title"));
...
CREATE INDEX "idx_quotation_activities_quotation_id" ON "public"."quotation_activities" USING "btree" ("quotation_id");
...
CREATE INDEX "idx_quotation_components_quotation_id" ON "public"."quotation_components" USING "btree" ("quotation_id");
...
CREATE INDEX "idx_quotation_line_items_quotation_id" ON "public"."quotation_line_items" USING "btree" ("quotation_id");
...
CREATE INDEX "idx_quotation_spaces_quotation_id" ON "public"."quotation_spaces" USING "btree" ("quotation_id");
...
CREATE INDEX "idx_quotation_template_line_items_template_id" ON "public"."quotation_template_line_items" USING "btree" ("template_id");
...
CREATE INDEX "idx_quotation_templates_level" ON "public"."quotation_templates" USING "btree" ("tenant_id", "level");
...
CREATE INDEX "idx_quotation_templates_tenant_id" ON "public"."quotation_templates" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_space_types_tenant_id" ON "public"."space_types" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_stock_brands_tenant_id" ON "public"."stock_brands" USING "btree" ("tenant_id");



CREATE INDEX "idx_stock_goods_receipts_tenant_id" ON "public"."stock_goods_receipts" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_stock_purchase_order_items_po_id" ON "public"."stock_purchase_order_items" USING "btree" ("po_id");



CREATE INDEX "idx_stock_purchase_orders_tenant_id" ON "public"."stock_purchase_orders" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_stock_vendor_brands_tenant_id" ON "public"."stock_vendor_brands" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_stock_vendors_tenant_id" ON "public"."stock_vendors" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_task_requirements_task" ON "public"."task_completion_requirements" USING "btree" ("task_id", "is_satisfied");



CREATE INDEX "idx_task_status_history_task" ON "public"."task_status_history" USING "btree" ("task_id", "changed_at");



CREATE INDEX "idx_task_status_history_tenant" ON "public"."task_status_history" USING "btree" ("tenant_id", "changed_at");



CREATE INDEX "idx_task_status_history_to" ON "public"."task_status_history" USING "btree" ("to_status");
...
CREATE INDEX "idx_task_tag_assignments_task_id" ON "public"."task_tag_assignments" USING "btree" ("task_id");



CREATE UNIQUE INDEX "idx_task_tag_assignments_unique" ON "public"."task_tag_assignments" USING "btree" ("task_id", "tag_id");
...
CREATE UNIQUE INDEX "idx_task_tags_tenant_name_unique" ON "public"."task_tags" USING "btree" ("tenant_id", "lower"(("name")::"text"));
...
CREATE UNIQUE INDEX "idx_task_work_sessions_open_unique" ON "public"."task_work_sessions" USING "btree" ("task_id", "user_id") WHERE ("ended_at" IS NULL);



CREATE INDEX "idx_task_work_sessions_task" ON "public"."task_work_sessions" USING "btree" ("task_id", "started_at");



CREATE INDEX "idx_task_work_sessions_tenant" ON "public"."task_work_sessions" USING "btree" ("tenant_id", "started_at");



CREATE INDEX "idx_task_work_sessions_user" ON "public"."task_work_sessions" USING "btree" ("user_id", "started_at");
...
CREATE INDEX "idx_tasks_procedure_run" ON "public"."tasks" USING "btree" ("procedure_run_id");
...
CREATE INDEX "idx_template_spaces_template_id" ON "public"."template_spaces" USING "btree" ("template_id");
...
CREATE INDEX "idx_users_id_tenant_id" ON "public"."users" USING "btree" ("id", "tenant_id");
...
CREATE OR REPLACE TRIGGER "project_activities_updated_at_trigger" BEFORE UPDATE ON "public"."project_activities" FOR EACH ROW EXECUTE FUNCTION "public"."update_project_activities_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_documents_satisfy_upload" AFTER INSERT ON "public"."documents" FOR EACH ROW EXECUTE FUNCTION "public"."trg_satisfy_upload_requirement"();



CREATE OR REPLACE TRIGGER "trg_documents_storage" AFTER INSERT OR DELETE OR UPDATE OF "file_size" ON "public"."documents" FOR EACH ROW EXECUTE FUNCTION "public"."trg_recalc_tenant_storage"();



CREATE OR REPLACE TRIGGER "trg_lead_notes_follow_up" AFTER INSERT OR DELETE OR UPDATE OF "follow_up_at", "follow_up_done_at" ON "public"."lead_notes" FOR EACH ROW EXECUTE FUNCTION "public"."trg_lead_note_follow_up"();
...
CREATE OR REPLACE TRIGGER "trg_project_notes_follow_up" AFTER INSERT OR DELETE OR UPDATE OF "follow_up_at", "follow_up_done_at" ON "public"."project_notes" FOR EACH ROW EXECUTE FUNCTION "public"."trg_project_note_follow_up"();



CREATE OR REPLACE TRIGGER "trg_psi_touch" BEFORE UPDATE ON "public"."property_scope_items" FOR EACH ROW EXECUTE FUNCTION "public"."touch_updated_at"();



CREATE OR REPLACE TRIGGER "trg_qpf_touch" BEFORE UPDATE ON "public"."quotation_print_formats" FOR EACH ROW EXECUTE FUNCTION "public"."touch_updated_at"();



CREATE OR REPLACE TRIGGER "trg_qtc_touch" BEFORE UPDATE ON "public"."quotation_terms_clauses" FOR EACH ROW EXECUTE FUNCTION "public"."touch_updated_at"();
...
CREATE OR REPLACE TRIGGER "trg_task_attachments_storage" AFTER INSERT OR DELETE OR UPDATE OF "file_size" ON "public"."task_attachments" FOR EACH ROW EXECUTE FUNCTION "public"."trg_recalc_tenant_storage"();



CREATE OR REPLACE TRIGGER "trg_tasks_cascade_link" AFTER UPDATE OF "related_type", "related_id" ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "public"."trg_tasks_cascade_link_to_subtasks"();



CREATE OR REPLACE TRIGGER "trg_tasks_inherit_parent_link" BEFORE INSERT ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "public"."trg_tasks_inherit_parent_link"();



CREATE OR REPLACE TRIGGER "trg_tasks_status_change" BEFORE UPDATE OF "status" ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "public"."trg_tasks_status_change"();
...
CREATE OR REPLACE TRIGGER "trigger_prevent_locked_quotation_edit" BEFORE UPDATE ON "public"."quotations" FOR EACH ROW EXECUTE FUNCTION "public"."prevent_locked_quotation_edit"();



CREATE OR REPLACE TRIGGER "trigger_project_notes_updated_at" BEFORE UPDATE ON "public"."project_notes" FOR EACH ROW EXECUTE FUNCTION "public"."update_project_notes_timestamp"();
...
ALTER TABLE ONLY "public"."project_notes"
    ADD CONSTRAINT "fk_project_notes_created_by" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."project_notes"
    ADD CONSTRAINT "fk_project_notes_project" FOREIGN KEY ("project_id") REFERENCES "public"."projects"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."project_notes"
    ADD CONSTRAINT "fk_project_notes_tenant" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."procedure_definitions"
    ADD CONSTRAINT "procedure_definitions_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."procedure_runs"
    ADD CONSTRAINT "procedure_runs_definition_id_fkey" FOREIGN KEY ("definition_id") REFERENCES "public"."procedure_definitions"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."procedure_runs"
    ADD CONSTRAINT "procedure_runs_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."procedure_step_definitions"
    ADD CONSTRAINT "procedure_step_definitions_definition_id_fkey" FOREIGN KEY ("definition_id") REFERENCES "public"."procedure_definitions"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."procedure_step_definitions"
    ADD CONSTRAINT "procedure_step_definitions_parent_step_id_fkey" FOREIGN KEY ("parent_step_id") REFERENCES "public"."procedure_step_definitions"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."project_activities"
    ADD CONSTRAINT "project_activities_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id");
...
ALTER TABLE ONLY "public"."projects"
    ADD CONSTRAINT "projects_baseline_quotation_fk" FOREIGN KEY ("baseline_quotation_id") REFERENCES "public"."quotations"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."projects"
    ADD CONSTRAINT "projects_current_phase_id_fkey" FOREIGN KEY ("current_phase_id") REFERENCES "public"."project_phases"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_component_type_id_fkey" FOREIGN KEY ("component_type_id") REFERENCES "public"."component_types"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "public"."property_scope_items"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_property_id_fkey" FOREIGN KEY ("property_id") REFERENCES "public"."properties"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_space_type_id_fkey" FOREIGN KEY ("space_type_id") REFERENCES "public"."space_types"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."property_scope_items"
    ADD CONSTRAINT "property_scope_items_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."quotation_print_formats"
    ADD CONSTRAINT "quotation_print_formats_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."quotation_print_formats"
    ADD CONSTRAINT "quotation_print_formats_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."quotation_print_formats"
    ADD CONSTRAINT "quotation_print_formats_terms_clause_id_fkey" FOREIGN KEY ("terms_clause_id") REFERENCES "public"."quotation_terms_clauses"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."quotation_terms_clauses"
    ADD CONSTRAINT "quotation_terms_clauses_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id");



ALTER TABLE ONLY "public"."quotation_terms_clauses"
    ADD CONSTRAINT "quotation_terms_clauses_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."quotations"
    ADD CONSTRAINT "quotations_linked_to_project_fk" FOREIGN KEY ("linked_to_project_id") REFERENCES "public"."projects"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."task_completion_requirements"
    ADD CONSTRAINT "task_completion_requirements_task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "public"."tasks"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."task_status_history"
    ADD CONSTRAINT "task_status_history_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."task_work_sessions"
    ADD CONSTRAINT "task_work_sessions_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."task_work_sessions"
    ADD CONSTRAINT "task_work_sessions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."tasks"
    ADD CONSTRAINT "tasks_procedure_run_id_fkey" FOREIGN KEY ("procedure_run_id") REFERENCES "public"."procedure_runs"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tasks"
    ADD CONSTRAINT "tasks_procedure_step_id_fkey" FOREIGN KEY ("procedure_step_id") REFERENCES "public"."procedure_step_definitions"("id") ON DELETE SET NULL;
...
CREATE POLICY "Brands are readable by authenticated users" ON "public"."stock_brands" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_brands"."tenant_id")))));



CREATE POLICY "Brands can be deleted by authenticated users" ON "public"."stock_brands" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_brands"."tenant_id")))));



CREATE POLICY "Brands can be inserted by authenticated users" ON "public"."stock_brands" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_brands"."tenant_id")))));



CREATE POLICY "Brands can be updated by authenticated users" ON "public"."stock_brands" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_brands"."tenant_id")))));



CREATE POLICY "Calendar events are readable by tenant" ON "public"."calendar_events" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "calendar_events"."tenant_id")))));



CREATE POLICY "Calendar events can be inserted by tenant" ON "public"."calendar_events" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "calendar_events"."tenant_id")))));



CREATE POLICY "Calendar events can be updated by tenant" ON "public"."calendar_events" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "calendar_events"."tenant_id")))));



CREATE POLICY "Clients are readable" ON "public"."clients" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "clients"."tenant_id")))));



CREATE POLICY "Clients can be deleted" ON "public"."clients" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "clients"."tenant_id")))));



CREATE POLICY "Clients can be inserted" ON "public"."clients" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "clients"."tenant_id")))));



CREATE POLICY "Clients can be updated" ON "public"."clients" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "clients"."tenant_id")))));



CREATE POLICY "Component types are readable by authenticated users" ON "public"."component_types" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "component_types"."tenant_id")))));



CREATE POLICY "Component types can be deleted by authenticated users" ON "public"."component_types" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "component_types"."tenant_id")))));



CREATE POLICY "Component types can be inserted by authenticated users" ON "public"."component_types" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "component_types"."tenant_id")))));



CREATE POLICY "Component types can be updated by authenticated users" ON "public"."component_types" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "component_types"."tenant_id")))));



CREATE POLICY "Cost attribute types are readable by authenticated users" ON "public"."cost_attribute_types" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "cost_attribute_types"."tenant_id")))));



CREATE POLICY "Cost attribute types can be deleted by authenticated users" ON "public"."cost_attribute_types" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "cost_attribute_types"."tenant_id")))));



CREATE POLICY "Cost attribute types can be inserted by authenticated users" ON "public"."cost_attribute_types" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "cost_attribute_types"."tenant_id")))));



CREATE POLICY "Cost attribute types can be updated by authenticated users" ON "public"."cost_attribute_types" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "cost_attribute_types"."tenant_id")))));
...
CREATE POLICY "Goods receipts are readable" ON "public"."stock_goods_receipts" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_goods_receipts"."tenant_id")))));



CREATE POLICY "Goods receipts can be deleted" ON "public"."stock_goods_receipts" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_goods_receipts"."tenant_id")))));



CREATE POLICY "Goods receipts can be inserted" ON "public"."stock_goods_receipts" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_goods_receipts"."tenant_id")))));



CREATE POLICY "Goods receipts can be updated" ON "public"."stock_goods_receipts" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_goods_receipts"."tenant_id")))));
...
CREATE POLICY "Lead activities are readable" ON "public"."lead_activities" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."leads" "l"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "l"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("l"."id" = "lead_activities"."lead_id")))));



CREATE POLICY "Lead activities can be inserted" ON "public"."lead_activities" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."leads" "l"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "l"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("l"."id" = "lead_activities"."lead_id")))));
...
CREATE POLICY "Lead notes are readable" ON "public"."lead_notes" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."leads" "l"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "l"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("l"."id" = "lead_notes"."lead_id")))));



CREATE POLICY "Lead notes can be deleted" ON "public"."lead_notes" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."leads" "l"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "l"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("l"."id" = "lead_notes"."lead_id")))));
...
CREATE POLICY "Lead notes can be inserted" ON "public"."lead_notes" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."leads" "l"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "l"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("l"."id" = "lead_notes"."lead_id")))));
...
CREATE POLICY "Lead notes can be updated" ON "public"."lead_notes" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."leads" "l"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "l"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("l"."id" = "lead_notes"."lead_id")))));
...
CREATE POLICY "Leads are readable" ON "public"."leads" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "leads"."tenant_id")))));



CREATE POLICY "Leads can be deleted" ON "public"."leads" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "leads"."tenant_id")))));



CREATE POLICY "Leads can be inserted" ON "public"."leads" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "leads"."tenant_id")))));



CREATE POLICY "Leads can be updated" ON "public"."leads" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "leads"."tenant_id")))));
...
CREATE POLICY "Phase activity logs are readable" ON "public"."project_phase_activity_log" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."projects" "p"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "p"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("p"."id" = "project_phase_activity_log"."project_id")))));



CREATE POLICY "Phase activity logs can be inserted" ON "public"."project_phase_activity_log" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."projects" "p"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "p"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("p"."id" = "project_phase_activity_log"."project_id")))));
...
CREATE POLICY "Project activities are readable" ON "public"."project_activities" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."projects" "p"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "p"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("p"."id" = "project_activities"."project_id")))));



CREATE POLICY "Project activities can be inserted" ON "public"."project_activities" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."projects" "p"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "p"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("p"."id" = "project_activities"."project_id")))));
...
CREATE POLICY "Projects are readable" ON "public"."projects" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "projects"."tenant_id")))));



CREATE POLICY "Projects can be deleted" ON "public"."projects" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "projects"."tenant_id")))));



CREATE POLICY "Projects can be inserted" ON "public"."projects" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "projects"."tenant_id")))));



CREATE POLICY "Projects can be updated" ON "public"."projects" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "projects"."tenant_id")))));



CREATE POLICY "Properties are readable" ON "public"."properties" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "properties"."tenant_id")))));



CREATE POLICY "Properties can be deleted" ON "public"."properties" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "properties"."tenant_id")))));



CREATE POLICY "Properties can be inserted" ON "public"."properties" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "properties"."tenant_id")))));



CREATE POLICY "Properties can be updated" ON "public"."properties" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "properties"."tenant_id")))));



CREATE POLICY "Purchase order items are readable" ON "public"."stock_purchase_order_items" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."stock_purchase_orders" "po"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "po"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("po"."id" = "stock_purchase_order_items"."po_id")))));



CREATE POLICY "Purchase order items can be deleted" ON "public"."stock_purchase_order_items" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."stock_purchase_orders" "po"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "po"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("po"."id" = "stock_purchase_order_items"."po_id")))));
...
CREATE POLICY "Purchase order items can be inserted" ON "public"."stock_purchase_order_items" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."stock_purchase_orders" "po"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "po"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("po"."id" = "stock_purchase_order_items"."po_id")))));
...
CREATE POLICY "Purchase order items can be updated" ON "public"."stock_purchase_order_items" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."stock_purchase_orders" "po"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "po"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("po"."id" = "stock_purchase_order_items"."po_id")))));
...
CREATE POLICY "Purchase orders are readable by authenticated users" ON "public"."stock_purchase_orders" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_purchase_orders"."tenant_id")))));



CREATE POLICY "Purchase orders can be deleted" ON "public"."stock_purchase_orders" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_purchase_orders"."tenant_id")))));



CREATE POLICY "Purchase orders can be inserted" ON "public"."stock_purchase_orders" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_purchase_orders"."tenant_id")))));



CREATE POLICY "Purchase orders can be updated" ON "public"."stock_purchase_orders" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_purchase_orders"."tenant_id")))));



CREATE POLICY "Quotation activities are readable" ON "public"."quotation_activities" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_activities"."quotation_id")))));



CREATE POLICY "Quotation activities can be inserted" ON "public"."quotation_activities" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_activities"."quotation_id")))));
...
CREATE POLICY "Quotation components are readable" ON "public"."quotation_components" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_components"."quotation_id")))));



CREATE POLICY "Quotation components can be deleted" ON "public"."quotation_components" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_components"."quotation_id")))));
...
CREATE POLICY "Quotation components can be inserted" ON "public"."quotation_components" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_components"."quotation_id")))));
...
CREATE POLICY "Quotation components can be updated" ON "public"."quotation_components" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_components"."quotation_id")))));
...
CREATE POLICY "Quotation line items are readable" ON "public"."quotation_line_items" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_line_items"."quotation_id")))));



CREATE POLICY "Quotation line items can be deleted" ON "public"."quotation_line_items" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_line_items"."quotation_id")))));
...
CREATE POLICY "Quotation line items can be inserted" ON "public"."quotation_line_items" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_line_items"."quotation_id")))));
...
CREATE POLICY "Quotation line items can be updated" ON "public"."quotation_line_items" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_line_items"."quotation_id")))));
...
CREATE POLICY "Quotation spaces are readable" ON "public"."quotation_spaces" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_spaces"."quotation_id")))));



CREATE POLICY "Quotation spaces can be deleted" ON "public"."quotation_spaces" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_spaces"."quotation_id")))));
...
CREATE POLICY "Quotation spaces can be inserted" ON "public"."quotation_spaces" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_spaces"."quotation_id")))));
...
CREATE POLICY "Quotation spaces can be updated" ON "public"."quotation_spaces" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotations" "q"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "q"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("q"."id" = "quotation_spaces"."quotation_id")))));
...
CREATE POLICY "Quotation templates are readable" ON "public"."quotation_templates" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates are readable by authenticated users" ON "public"."quotation_templates" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates can be deleted" ON "public"."quotation_templates" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates can be deleted by authenticated users" ON "public"."quotation_templates" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates can be inserted" ON "public"."quotation_templates" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates can be inserted by authenticated users" ON "public"."quotation_templates" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates can be updated" ON "public"."quotation_templates" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotation templates can be updated by authenticated users" ON "public"."quotation_templates" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotation_templates"."tenant_id")))));



CREATE POLICY "Quotations are readable" ON "public"."quotations" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotations"."tenant_id")))));



CREATE POLICY "Quotations can be deleted" ON "public"."quotations" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotations"."tenant_id")))));



CREATE POLICY "Quotations can be inserted" ON "public"."quotations" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotations"."tenant_id")))));



CREATE POLICY "Quotations can be updated" ON "public"."quotations" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "quotations"."tenant_id")))));
...
CREATE POLICY "Space types are readable by authenticated users" ON "public"."space_types" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "space_types"."tenant_id")))));



CREATE POLICY "Space types can be deleted by authenticated users" ON "public"."space_types" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "space_types"."tenant_id")))));



CREATE POLICY "Space types can be inserted by authenticated users" ON "public"."space_types" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "space_types"."tenant_id")))));



CREATE POLICY "Space types can be updated by authenticated users" ON "public"."space_types" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "space_types"."tenant_id")))));
...
CREATE POLICY "Task activities are readable" ON "public"."task_activities" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."tasks" "t"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "t"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("t"."id" = "task_activities"."task_id")))));



CREATE POLICY "Task activities can be inserted" ON "public"."task_activities" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."tasks" "t"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "t"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("t"."id" = "task_activities"."task_id")))));
...
CREATE POLICY "Task tag assignments are readable" ON "public"."task_tag_assignments" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."tasks" "t"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "t"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("t"."id" = "task_tag_assignments"."task_id")))));



CREATE POLICY "Task tag assignments can be deleted" ON "public"."task_tag_assignments" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."tasks" "t"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "t"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("t"."id" = "task_tag_assignments"."task_id")))));
...
CREATE POLICY "Task tag assignments can be inserted" ON "public"."task_tag_assignments" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."tasks" "t"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "t"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("t"."id" = "task_tag_assignments"."task_id")))));
...
CREATE POLICY "Task template items are readable" ON "public"."task_template_items" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));



CREATE POLICY "Task template items can be inserted" ON "public"."task_template_items" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));



CREATE POLICY "Tasks are readable" ON "public"."tasks" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "tasks"."tenant_id")))));



CREATE POLICY "Tasks can be deleted" ON "public"."tasks" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "tasks"."tenant_id")))));



CREATE POLICY "Tasks can be inserted" ON "public"."tasks" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "tasks"."tenant_id")))));



CREATE POLICY "Tasks can be updated" ON "public"."tasks" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "tasks"."tenant_id")))));



CREATE POLICY "Template line items are readable" ON "public"."quotation_template_line_items" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "quotation_template_line_items"."template_id")))));



CREATE POLICY "Template line items can be deleted" ON "public"."quotation_template_line_items" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "quotation_template_line_items"."template_id")))));
...
CREATE POLICY "Template line items can be inserted" ON "public"."quotation_template_line_items" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "quotation_template_line_items"."template_id")))));
...
CREATE POLICY "Template line items can be updated" ON "public"."quotation_template_line_items" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "quotation_template_line_items"."template_id")))));
...
CREATE POLICY "Template spaces are readable" ON "public"."template_spaces" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "template_spaces"."template_id")))));



CREATE POLICY "Template spaces can be deleted" ON "public"."template_spaces" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "template_spaces"."template_id")))));
...
CREATE POLICY "Template spaces can be inserted" ON "public"."template_spaces" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "template_spaces"."template_id")))));
...
CREATE POLICY "Template spaces can be updated" ON "public"."template_spaces" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM ("public"."quotation_templates" "qt"
     JOIN "public"."users" "u" ON (("u"."tenant_id" = "qt"."tenant_id")))
  WHERE (("u"."id" = "auth"."uid"()) AND ("qt"."id" = "template_spaces"."template_id")))));
...
CREATE POLICY "Users can see their own profile" ON "public"."users" FOR SELECT TO "authenticated" USING (("auth"."uid"() = "id"));
...
CREATE POLICY "Vendor brands are readable by authenticated users" ON "public"."stock_vendor_brands" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendor_brands"."tenant_id")))));



CREATE POLICY "Vendor brands can be deleted by authenticated users" ON "public"."stock_vendor_brands" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendor_brands"."tenant_id")))));



CREATE POLICY "Vendor brands can be inserted by authenticated users" ON "public"."stock_vendor_brands" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendor_brands"."tenant_id")))));



CREATE POLICY "Vendor brands can be updated by authenticated users" ON "public"."stock_vendor_brands" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendor_brands"."tenant_id")))));



CREATE POLICY "Vendors are readable by authenticated users" ON "public"."stock_vendors" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendors"."tenant_id")))));



CREATE POLICY "Vendors can be deleted by authenticated users" ON "public"."stock_vendors" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendors"."tenant_id")))));



CREATE POLICY "Vendors can be inserted by authenticated users" ON "public"."stock_vendors" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendors"."tenant_id")))));



CREATE POLICY "Vendors can be updated by authenticated users" ON "public"."stock_vendors" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."tenant_id" = "stock_vendors"."tenant_id")))));
...
ALTER TABLE "public"."cost_attribute_types" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "procedure_definitions_access" ON "public"."procedure_definitions" USING (("tenant_id" = "public"."get_user_tenant_id"()));
...
CREATE POLICY "procedure_runs_access" ON "public"."procedure_runs" USING (("tenant_id" = "public"."get_user_tenant_id"()));
...
CREATE POLICY "procedure_step_definitions_access" ON "public"."procedure_step_definitions" USING (("definition_id" IN ( SELECT "procedure_definitions"."id"
   FROM "public"."procedure_definitions"
  WHERE ("procedure_definitions"."tenant_id" = "public"."get_user_tenant_id"()))));
...
CREATE POLICY "project_activities_delete_policy" ON "public"."project_activities" FOR DELETE USING (((EXISTS ( SELECT 1
   FROM "public"."projects"
  WHERE (("projects"."id" = "project_activities"."project_id") AND ("projects"."tenant_id" = "public"."get_user_tenant_id"())))) AND ("created_by" = "auth"."uid"())));



CREATE POLICY "project_activities_insert_policy" ON "public"."project_activities" FOR INSERT WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."projects"
  WHERE (("projects"."id" = "project_activities"."project_id") AND ("projects"."tenant_id" = "public"."get_user_tenant_id"())))) AND ("created_by" = "auth"."uid"())));



CREATE POLICY "project_activities_select_policy" ON "public"."project_activities" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."projects"
  WHERE (("projects"."id" = "project_activities"."project_id") AND ("projects"."tenant_id" = "public"."get_user_tenant_id"())))));



CREATE POLICY "project_activities_update_policy" ON "public"."project_activities" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."projects"
  WHERE (("projects"."id" = "project_activities"."project_id") AND ("projects"."tenant_id" = "public"."get_user_tenant_id"())))));
...
CREATE POLICY "project_notes_access" ON "public"."project_notes" USING ((EXISTS ( SELECT 1
   FROM "public"."projects"
  WHERE (("projects"."id" = "project_notes"."project_id") AND ("projects"."tenant_id" = "public"."get_user_tenant_id"()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."projects"
  WHERE (("projects"."id" = "project_notes"."project_id") AND ("projects"."tenant_id" = "public"."get_user_tenant_id"())))));
...
CREATE POLICY "psi_tenant_access" ON "public"."property_scope_items" TO "authenticated" USING (("tenant_id" IN ( SELECT "users"."tenant_id"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"())))) WITH CHECK (("tenant_id" IN ( SELECT "users"."tenant_id"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));
...
CREATE POLICY "qpf_tenant_access" ON "public"."quotation_print_formats" TO "authenticated" USING (("tenant_id" IN ( SELECT "users"."tenant_id"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"())))) WITH CHECK (("tenant_id" IN ( SELECT "users"."tenant_id"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));
...
CREATE POLICY "qtc_tenant_access" ON "public"."quotation_terms_clauses" TO "authenticated" USING (("tenant_id" IN ( SELECT "users"."tenant_id"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"())))) WITH CHECK (("tenant_id" IN ( SELECT "users"."tenant_id"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"()))));
...
CREATE POLICY "task_completion_requirements_access" ON "public"."task_completion_requirements" USING (("task_id" IN ( SELECT "tasks"."id"
   FROM "public"."tasks"
  WHERE ("tasks"."tenant_id" = "public"."get_user_tenant_id"()))));
...
CREATE POLICY "task_status_history_access" ON "public"."task_status_history" USING (("task_id" IN ( SELECT "tasks"."id"
   FROM "public"."tasks"
  WHERE ("tasks"."tenant_id" = "public"."get_user_tenant_id"()))));
...
CREATE POLICY "task_work_sessions_access" ON "public"."task_work_sessions" USING (("task_id" IN ( SELECT "tasks"."id"
   FROM "public"."tasks"
  WHERE ("tasks"."tenant_id" = "public"."get_user_tenant_id"()))));
...
GRANT ALL ON FUNCTION "public"."can_complete_task"("p_task_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."can_complete_task"("p_task_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."can_complete_task"("p_task_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."can_start_task"("p_task_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."can_start_task"("p_task_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."can_start_task"("p_task_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."copy_quotation_to_project"("p_source_quotation_id" "uuid", "p_project_id" "uuid", "p_created_by" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."copy_quotation_to_project"("p_source_quotation_id" "uuid", "p_project_id" "uuid", "p_created_by" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."copy_quotation_to_project"("p_source_quotation_id" "uuid", "p_project_id" "uuid", "p_created_by" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_project_from_lead"("p_lead_id" "uuid", "p_created_by" "uuid", "p_project_category" "text", "p_initialize_phases" boolean, "p_quotation_id" "uuid", "p_project_manager_id" "uuid", "p_priority" "text", "p_target_start_date" "date", "p_target_end_date" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."create_project_from_lead"("p_lead_id" "uuid", "p_created_by" "uuid", "p_project_category" "text", "p_initialize_phases" boolean, "p_quotation_id" "uuid", "p_project_manager_id" "uuid", "p_priority" "text", "p_target_start_date" "date", "p_target_end_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_project_from_lead"("p_lead_id" "uuid", "p_created_by" "uuid", "p_project_category" "text", "p_initialize_phases" boolean, "p_quotation_id" "uuid", "p_project_manager_id" "uuid", "p_priority" "text", "p_target_start_date" "date", "p_target_end_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_quotation_from_lead"("p_lead_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."create_quotation_from_lead"("p_lead_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_quotation_from_lead"("p_lead_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."is_valid_task_transition"("p_from" "public"."task_status", "p_to" "public"."task_status") TO "anon";
GRANT ALL ON FUNCTION "public"."is_valid_task_transition"("p_from" "public"."task_status", "p_to" "public"."task_status") TO "authenticated";
GRANT ALL ON FUNCTION "public"."is_valid_task_transition"("p_from" "public"."task_status", "p_to" "public"."task_status") TO "service_role";



GRANT ALL ON FUNCTION "public"."lock_quotation_for_project"("p_quotation_id" "uuid", "p_project_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."lock_quotation_for_project"("p_quotation_id" "uuid", "p_project_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."lock_quotation_for_project"("p_quotation_id" "uuid", "p_project_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."recalc_lead_next_follow_up"("p_lead_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."recalc_lead_next_follow_up"("p_lead_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."recalc_lead_next_follow_up"("p_lead_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."recalc_project_next_follow_up"("p_project_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."recalc_project_next_follow_up"("p_project_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."recalc_project_next_follow_up"("p_project_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."recalc_task_timing"("p_task_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."recalc_task_timing"("p_task_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."recalc_task_timing"("p_task_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."recalc_tenant_storage"("p_tenant_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."recalc_tenant_storage"("p_tenant_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."recalc_tenant_storage"("p_tenant_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."revoke_requirement_signoff"("p_requirement_id" "uuid", "p_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."revoke_requirement_signoff"("p_requirement_id" "uuid", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."revoke_requirement_signoff"("p_requirement_id" "uuid", "p_user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."sign_off_requirement"("p_requirement_id" "uuid", "p_user_id" "uuid", "p_note" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."sign_off_requirement"("p_requirement_id" "uuid", "p_user_id" "uuid", "p_note" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."sign_off_requirement"("p_requirement_id" "uuid", "p_user_id" "uuid", "p_note" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."start_procedure_run"("p_definition_id" "uuid", "p_related_type" "public"."task_related_type", "p_related_id" "uuid", "p_user_id" "uuid", "p_start_date" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."start_procedure_run"("p_definition_id" "uuid", "p_related_type" "public"."task_related_type", "p_related_id" "uuid", "p_user_id" "uuid", "p_start_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."start_procedure_run"("p_definition_id" "uuid", "p_related_type" "public"."task_related_type", "p_related_id" "uuid", "p_user_id" "uuid", "p_start_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."task_blocking_predecessors"("p_task_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."task_blocking_predecessors"("p_task_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."task_blocking_predecessors"("p_task_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."task_transition"("p_task_id" "uuid", "p_user_id" "uuid", "p_to" "public"."task_status", "p_reason" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."task_transition"("p_task_id" "uuid", "p_user_id" "uuid", "p_to" "public"."task_status", "p_reason" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."task_transition"("p_task_id" "uuid", "p_user_id" "uuid", "p_to" "public"."task_status", "p_reason" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."trg_lead_note_follow_up"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_lead_note_follow_up"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_lead_note_follow_up"() TO "service_role";



GRANT ALL ON FUNCTION "public"."trg_project_note_follow_up"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_project_note_follow_up"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_project_note_follow_up"() TO "service_role";



GRANT ALL ON FUNCTION "public"."trg_recalc_tenant_storage"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_recalc_tenant_storage"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_recalc_tenant_storage"() TO "service_role";



GRANT ALL ON FUNCTION "public"."trg_satisfy_upload_requirement"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_satisfy_upload_requirement"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_satisfy_upload_requirement"() TO "service_role";



GRANT ALL ON FUNCTION "public"."trg_tasks_cascade_link_to_subtasks"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_tasks_cascade_link_to_subtasks"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_tasks_cascade_link_to_subtasks"() TO "service_role";



GRANT ALL ON FUNCTION "public"."trg_tasks_inherit_parent_link"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_tasks_inherit_parent_link"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_tasks_inherit_parent_link"() TO "service_role";



GRANT ALL ON FUNCTION "public"."trg_tasks_status_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."trg_tasks_status_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."trg_tasks_status_change"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_project_activities_updated_at"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_project_activities_updated_at"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_project_activities_updated_at"() TO "service_role";



GRANT ALL ON FUNCTION "public"."update_project_notes_timestamp"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_project_notes_timestamp"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_project_notes_timestamp"() TO "service_role";
...
GRANT ALL ON TABLE "public"."procedure_definitions" TO "anon";
GRANT ALL ON TABLE "public"."procedure_definitions" TO "authenticated";
GRANT ALL ON TABLE "public"."procedure_definitions" TO "service_role";
...
GRANT ALL ON TABLE "public"."procedure_step_definitions" TO "anon";
GRANT ALL ON TABLE "public"."procedure_step_definitions" TO "authenticated";
GRANT ALL ON TABLE "public"."procedure_step_definitions" TO "service_role";
...
GRANT ALL ON TABLE "public"."property_scope_items" TO "anon";
GRANT ALL ON TABLE "public"."property_scope_items" TO "authenticated";
GRANT ALL ON TABLE "public"."property_scope_items" TO "service_role";
...
GRANT ALL ON TABLE "public"."quotation_print_formats" TO "anon";
GRANT ALL ON TABLE "public"."quotation_print_formats" TO "authenticated";
GRANT ALL ON TABLE "public"."quotation_print_formats" TO "service_role";
...
GRANT ALL ON TABLE "public"."quotation_terms_clauses" TO "anon";
GRANT ALL ON TABLE "public"."quotation_terms_clauses" TO "authenticated";
GRANT ALL ON TABLE "public"."quotation_terms_clauses" TO "service_role";
...
GRANT ALL ON TABLE "public"."task_completion_requirements" TO "anon";
GRANT ALL ON TABLE "public"."task_completion_requirements" TO "authenticated";
GRANT ALL ON TABLE "public"."task_completion_requirements" TO "service_role";
