-- A started step's dates are the real ones.
--
-- The plan showed a planned start in the future on a step that had already
-- been started. Once a step has begun, the current plan follows what
-- happened: its start is the day it began, its due is that day plus its
-- duration (no earlier than today), and a finished step shows the dates it
-- actually ran between. The agreed plan (plan_baselines) keeps the original
-- dates, so early and late remain measurable. Pinned dates are left alone.
--
-- This replaces the earlier rule that a transition never rewrote start_date -
-- that rule predates the baseline, which is now where the plan is kept.
