-- Nothing goes in progress without an owner - Start, Resume or Reopen alike.
-- 20260916090000 applied the rule only from `todo`, so a paused step whose
-- assignee had been cleared could still be resumed.
...
BEGIN
    ...
    IF v_task.assigned_to IS NULL AND v_task.status IN ('todo', 'on_hold', 'blocked', 'completed', 'cancelled') THEN
    ...
    IF v_needs_start IS NOT NULL AND array_length(v_needs_start, 1) > 0 THEN
        v_parts := v_parts || format('%s to start', array_to_string(v_needs_start, ', '));
    END IF;
    IF v_needs_finish IS NOT NULL AND array_length(v_needs_finish, 1) > 0 THEN
        v_parts := v_parts || format('%s to finish', array_to_string(v_needs_finish, ', '));
    END IF;
