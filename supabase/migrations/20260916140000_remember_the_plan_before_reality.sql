-- A step remembers what was planned before reality replaced it.
--
-- Now that a started step's start and due are the real dates, the plan as it
-- stood when the step began has to be kept somewhere, or "started early" and
-- "finished late" have nothing to compare against on a project that was
-- never kicked off (no baseline). planned_start_date / planned_due_date are
-- stamped once, by the scheduler, the first time it sees the step started.

ALTER TABLE "public"."tasks"
  ADD COLUMN IF NOT EXISTS "planned_start_date" date,
  ADD COLUMN IF NOT EXISTS "planned_due_date" date;
COMMENT ON COLUMN "public"."tasks"."planned_start_date" IS 'The planned start as it stood when the task began. Stamped once by reschedule_run.';
COMMENT ON COLUMN "public"."tasks"."planned_due_date" IS 'The planned due date as it stood when the task began. Stamped once by reschedule_run.';
