-- Express a playbook step in hours, and derive the date from it.
--
-- The builder asked for two numbers: days to finish, which set the due date,
-- and hours of work, which was carried onto the task. They are genuinely
-- different things and that is exactly why asking for both was wrong - it made
-- people choose between them, and the one that matters for improving is hours,
-- because hours are what actual_hours can be compared against.
--
-- So the playbook is written in hours. A due date still has to be a date, so
-- it is derived at eight hours to a working day, rounded up, never less than
-- one. An explicit duration_days still wins where a playbook already set one,
-- so nothing already configured changes meaning.
