-- Two scheduler corrections from the v8 lay-out.
--
-- 1. The project's planned start anchors only the first stage. It was a floor
--    for every stage, so 3D Design could not start on 30 Sep even though 2D
--    finished on the 29th, because the original sales date said 1 Oct.
-- 2. Skipped and cancelled steps take no time. A step cancelled by a commit
--    kept its planned dates and stretched its stage by two days; nothing
--    should wait for work that will not happen.
