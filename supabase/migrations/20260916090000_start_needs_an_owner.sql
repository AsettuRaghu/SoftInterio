-- A task cannot be started without an assignee.
--
-- Start was available on a step nobody owned, which put work in progress
-- that no one was doing. can_start_task now answers "Assign someone first"
-- (so the Plan tab disables Start and prints the reason on the row) and
-- task_transition refuses the same move, so no client can slip past it.
-- Both functions otherwise as their previous definitions.
...
BEGIN
    ...
    -- Work has an owner before it has a start. A step nobody is assigned to
    -- cannot be started; the row says so and the button is disabled.
    IF v_task.assigned_to IS NULL AND v_task.status = 'todo' THEN
        RETURN jsonb_build_object(
            'can_start', false,
            'reason', 'Assign someone first',
            'blocking_predecessors', '[]'::jsonb
        );
    END IF;
...
BEGIN
    ...
    IF p_to = 'in_progress' AND v_task.status = 'todo' AND v_task.assigned_to IS NULL THEN
