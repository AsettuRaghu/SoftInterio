-- Two corrections to the scheduler after its first real plan.
--
-- 1. A step's floor is its own stage's start, not the project's planned
--    start. Steps inside a stage already under way were being pushed to the
--    project's original start date instead of following the stage.
-- 2. The implicit "after the previous sibling" rule yields when that sibling
--    explicitly waits for this step. "All 2D Reworks waits for 2D Client
--    Approval" was typed above Approval in the playbook, so each rule pushed
--    the other one pass at a time until the fixed-point loop gave up at 80
--    passes - eight months into next year. A warning is raised if the loop
--    ever fails to settle again.
...
BEGIN
  ...
  FOR v_pass IN 1..80 LOOP
    ...
    FOR r IN SELECT * FROM _plan WHERE NOT fixed ORDER BY (parent IS NOT NULL), ord LOOP
      ...
      -- After the previous sibling, or alongside it if Parallel - unless that
      -- sibling explicitly waits for THIS step, in which case the playbook has
      -- said the order and the list position is only where it was typed.
      -- Without this exception "Reworks waits for Client Approval", typed
      -- above Approval, chased its own tail until the plan ran into next year.
      SELECT GREATEST(v_start, COALESCE(MAX(x), v_start)) INTO v_start
        FROM (SELECT CASE WHEN r.parallel THEN p.s ELSE p.e + 1 END AS x
               ...
               WHERE p.parent IS NOT DISTINCT FROM r.parent AND p.ord < r.ord AND p.id <> r.id
                 AND NOT EXISTS (SELECT 1 FROM public.procedure_step_dependencies d
                                  WHERE d.step_id = p.step_id AND d.depends_on_step_id = r.step_id)
