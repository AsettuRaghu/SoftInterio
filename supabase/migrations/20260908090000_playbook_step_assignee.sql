-- Let a playbook step name the person who does it.
--
-- Steps could only name a role, and start_procedure_run resolves a role to a
-- person only when exactly one user holds it - a deliberate refusal to guess,
-- but it means a step arrives unowned on any team with two designers. Naming
-- the person is the plainer decision, and it is what makes a playbook worth
-- configuring once: adopt it, start it, and the work is already on the right
-- desks.
--
-- assign_to_role stays. A named person wins; the role is the fallback.
--
-- ON DELETE SET NULL rather than CASCADE: someone leaving the company must not
-- silently delete a step from every playbook that named them. The step loses
-- its owner and the run assigns nobody, which is visible and fixable.

ALTER TABLE "public"."procedure_step_definitions"
  ADD COLUMN IF NOT EXISTS "assign_to_user" "uuid"
  REFERENCES "public"."users"("id") ON DELETE SET NULL;

COMMENT ON COLUMN "public"."procedure_step_definitions"."assign_to_user" IS
  'The person this step is for. Takes precedence over assign_to_role, which '
  'only resolves when exactly one user holds the role.';
