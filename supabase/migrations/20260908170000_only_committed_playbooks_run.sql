-- Only a committed playbook can be started.
--
-- The status column exists but nothing enforced it, so a draft could still be
-- run through the API and land half-written work on a project.
