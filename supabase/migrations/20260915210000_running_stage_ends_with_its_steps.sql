-- A running stage ends when its last step does; its previous due date is not
-- a floor. Otherwise a wrong value, once written, could never be corrected -
-- which is exactly what the cycle bug had left behind on 2D Designs.
...
BEGIN
  ...
  -- is not a floor, or a stale value would never be corrected.
