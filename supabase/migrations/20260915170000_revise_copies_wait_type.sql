-- Revising a playbook copies each dependency's wait_type.
--
-- revise_playbook was written before wait_type existed (20260909190000) and
-- copied only dependency_type, so every "wait for X to start" link became
-- "wait for X to finish" in the new draft. A step that waits for its own stage
-- to start is the normal case - and its copy, now waiting for the stage to
-- FINISH, is exactly the pair trg_reject_circular_step_dependency refuses. So
-- since 2026-09-13 revising any playbook with such a link failed with "A step
-- cannot wait for its phase to FINISH", which the lifecycle route reported as
-- "Could not open a revision". Nothing was lost - the function is one
-- transaction - but nothing could be revised either.
--
-- Same function as 20260915150000 with wait_type carried across.
